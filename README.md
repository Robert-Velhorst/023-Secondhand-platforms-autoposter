...
The latest browser-auth checkpoint passed **525 tests with one POSIX-only skip in 121.65 seconds on Windows on 2026-09-13** (526 cases), plus Ruff, compilation, and JavaScript syntax checks. One new pytest wrapper executes **eleven JavaScript state tests**; six separately executed Chromium flows cover temporary session failure/retry, single-flight sign-in, invalid registration, mobile recovery layout, slow dashboard loading, and failed/retried logout with real backend revocation. See [browser session recovery](docs/AUTH_SECURITY_POSTURE.md#browser-session-recovery) and [test layers and commands](docs/TESTING_STRATEGY.md#verification-gate). Production deployment, manual accessibility QA, and human acceptance remain separate requirements.

The previous registration checkpoint passed **524 tests with one POSIX-only skip in 144.08 seconds on Windows on 2026-09-13** (525 cases), plus Ruff and compilation. Nine new cases cover concurrent sign-ups, account/session rollback, duplicate fast paths, connection lifetime, unrelated database failures, and uncertain-commit recovery. See the [verification record](docs/FINAL_VERIFICATION_REPORT.md#atomic-registration-and-retry-recovery--2026-09-13) and [registration recovery guidance](docs/AUTH_SECURITY_POSTURE.md#registration-transactions-and-recovery).

The previous login-account-state checkpoint passed **515 tests with one POSIX-only skip in 152.56 seconds on Windows on 2026-09-13** (516 cases), plus Ruff and compilation. All **99 PostgreSQL subset checks** passed. Fourteen new cases cover disabled accounts, credential changes during login/rehash, atomic failure rollback, profile-change controls, and releasing database connections before password work. See the [verification record](docs/FINAL_VERIFICATION_REPORT.md#login-account-state-and-connection-lifetime--2026-09-13) and [account-state guarantees and limits](docs/AUTH_SECURITY_POSTURE.md#account-state-at-login).

The previous atomic-login checkpoint passed **501 tests with one POSIX-only skip in 139.72 seconds on Windows on 2026-09-13** (502 cases), plus Ruff and compilation. All **85 PostgreSQL subset checks** passed. Sixteen new cases cover concurrent admission, successful-completion fencing, error rollback, expiry cleanup, and migration preservation. See the [verification record](docs/FINAL_VERIFICATION_REPORT.md#atomic-login-admission-and-expiry-maintenance--2026-09-13).
...
GitHub's `postgres-workers` job is configured to run 108 job-safety, storage-cleanup, image-write recovery, login-admission, login-account-state, and registration-transaction cases against a disposable PostgreSQL 16 service. All 108 passed locally on 2026-09-13, including concurrent HTTP/DB admission, completion fencing, account/credential changes, connection lifetime, registration conflicts, rollback/recovery, expiry cleanup, and migration preservation. Each case migrates its own newly created schema to Alembic head and removes that schema afterward. These rerun a subset of the product tests against a different database; they are not 108 additional distinct cases or evidence of a deployed production database. Use the checks attached to the exact commit for GitHub CI status.
