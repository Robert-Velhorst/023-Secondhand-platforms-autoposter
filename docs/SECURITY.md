...
- Publishing-account selection: account IDs are resolved with both listing-owner and target-platform predicates at enqueue, retry, and adapter execution. Whole-request preflight rejects invalid selections before revision or queue writes; no-account assisted preparation remains supported.
...
| Foreign or wrong-platform publishing account | Shared owner/platform lookup, generic API rejection, worker revalidation including recovered jobs | Existing invalid job rows are not rewritten by a migration. This does not prove live credential use or provider authorization; current adapters remain assisted/manual. |
