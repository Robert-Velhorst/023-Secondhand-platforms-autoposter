...
| HAI feeds and consumer contract | `tests/test_hai_connector.py`, `scripts/hai_consumer_contract_test.go` | Twenty-five app-side cases cover connector tokens, owner isolation, incremental changes, generic downloads, byte limits including Go JSON escaping, and bounded query counts. The separate Go harness exercises a real download through the pinned HAI parser and in-memory ingestion; it is not part of pytest or proof of target-installation persistence. See [reproduction instructions](HAI_CONNECTOR.md#reproduce-the-real-consumer-contract-check). |
...
| Frontend delivery and charts | `tests/test_static_delivery.py`, `scripts/browser_dashboard_checks.js` | Seven HTTP cases verify revalidation on GET/HEAD/304, the actual browser icon, strict CSP retention, and replacement of an old cached file after an update. The separate Playwright CLI check exercises real chart rendering under the app's strict CSP, relative bar widths including zero, displayed counts, label escaping, and empty data. Run it against a signed-in isolated app with `playwright-cli run-code --filename scripts/browser_dashboard_checks.js`; it is not counted as a pytest case. |
